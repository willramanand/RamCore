...
| Ability targeting/telegraphs | `ability` (`AbilityTargets`, `AbilityGeometry`), `ability.telegraph` | Core/gameplay | Experimental | Target resolution reads live state on the caster's thread; cone/beam geometry is pure and unit-tested. Telegraphs and glow lock-on are server-side only (no client mod/HUD) — every particle/glow/marker render runs on the caster, entity, or region scheduler; `AbilityAiming` drives its render/glow/timeout through the player scheduler and wires confirm/cancel through the `Events` API. |
| Dialogue | `dialogue` (config type `dialogues`) | Core/gameplay | Experimental | Graph model and traversal are pure; sessions run on the player's thread. Chat uses Paper `ClickEvent.callback` (no command registration); menu uses `MenuView`. Presentation is injectable, so traversal is tested headlessly. |
| Hot reload | `reload` | Core/gameplay | Experimental | Load/diff/resolve run on the async scheduler; each rebind is dispatched to its live object's owner scheduler. Failures are collected, never thrown. Snapshot/diff/hashing are pure. |
| Scheduling (real-time) | `schedule` | Core/gameplay | Experimental | In-house cron parser and next-fire math are pure; the runner ticks on the async scheduler and runs each job on its declared TaskContext. Uses an injectable clock. |
| Instanced worlds | `worldinstance` (`PaperWorldBackend` in ramcore-paper) | Core/gameplay | Paper-experimental | File copy/marker/sweep run async and are unit-tested via an in-memory `WorldBackend` fake; world load/unload hop to the global region. Gated on `supportsInstances()` — refuses on Folia. |
