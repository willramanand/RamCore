...
**Outcome (done 2026-09-10, one commit on `phase/b-roadmap-1x`).** Landed as designed across both PRs. Economy: `Economy`/`EconomyResult`/`InMemoryEconomy`/`VaultEconomy`/`Economies.detect`; `net.milkbowl` is touched only inside a helper reached after the registry reports Vault available. Rewards: `RewardActions.money/command/message/item/permissionCheck`, `RewardSubjects.playerId/onlinePlayer`, `Rewards` facade; actions carry a fixed outcome id (the `RewardAction` signature has no access to the entry id). Placeholders: `PlaceholderProvider`, `PlaceholderRegistry` (key split at the first underscore; MiniMessage `tagResolver` per provider), `PlaceholderApiBridge` (one expansion per provider, `me.clip` guarded behind availability), `RamCorePlaceholders` builder. Region: added `RegionRuleEngine.regionsAt`/`region`, `RegionEnterEvent`/`RegionExitEvent`, and `RegionTracker` whose `transition`/`clear` core is unit-tested with a custom `TransitionHandler` (Bukkit event firing needs a live player). Deps added to the pom as `provided`: VaultAPI 1.7 (JitPack) and placeholderapi 2.11.5 (added the ExtendedClip repo id so the cached artifact resolves offline). 23 tests: InMemoryEconomy, money through RewardEngine, placeholder resolution/param parsing without PAPI, RegionTracker transitions, and clean-absence of both bridges via a no-plugin detector. Note for 2.2: `VaultEconomy` and `PlaceholderApiBridge` move to `ramcore-paper`; the abstractions stay in `ramcore-api`.
...
**Outcome (done 2026-09-10, three commits on `phase/c-engineering-quality`).** PR1 switched the build to Gradle Kotlin DSL reproducing the single shaded jar exactly (verified: relocations, no leaks, no provided deps, manifest and paper-plugin.yml correct, all tests green) and deleted `pom.xml`; Gradle 8.8 runs on JDK 21 and compiles with a Java 25 toolchain, and shadow's ASM is forced to 9.8 to remap Java 25 bytecode. PR2 split into `ramcore-api`/`-nms`/`-protocol`/`-kotlin`/`-paper`/`-test`; `ramcore-api` compiles standalone and `ApiBoundaryTest` moved into it. Boundary resolutions: `reflect` stayed in api (its value types are api-clean and `nms.api` needs them); the ProtocolLib provider is contributed through a `ServiceLoader` `IntegrationProviderFactory`; `Gui`'s dead import removed. Deviations from the plan mapping (all to keep api facades intact and boundary-clean): `SqlStore`, `VaultEconomy`, `PlaceholderApiBridge` and the `Paper*` backends stay in api because api facades/defaults reference them and their optional deps are compileOnly and not forbidden; non-boundary tests live in `ramcore-paper/src/test` rather than scattered per module; `ramcore-test` holds the fakes only (StoreContractTest stayed with the tests). PR3 added `maven-publish` to every module except `ramcore-paper`, a `jitpack.yml` (JDK 21 launcher, foojay-provisioned 25 toolchain), README consume/module sections and a RELEASE_READINESS module checklist. 382 tests green throughout. Remaining manual gate: live runPaper/runFolia smoke (Gradle run-paper wiring is a follow-up).
...
**Outcome (done 2026-09-10, one commit on `phase/d-gameplay`).** `session` package in api: `SessionRecorder` (NOOP + ring), `SessionEvent`, `SessionEventType`, `RingSessionRecorder` (fixed array per player, concurrent), `SessionTimelines` formatter. `/ramcore diagnostics timeline <player> [n]` prints through `DiagnosticExporter.safeLines`; `RamCoreConfig` gains `diagnostics.timeline.enabled` (false) and `RamCore` installs a ring recorder (tick from `Bukkit.getCurrentTick`) when enabled, passing it to the diagnostics module. Deviation: the per-subsystem instrumentation hooks (RewardListener, loot/objective/cooldown/region/menu/ability) are deferred; `record(...)` is the integration point and subsystems call it as they gain hooks. 5 tests (ring capacity, limit, clear, NOOP, formatting); full build 405 green.
...
**Outcome (done 2026-09-11, three commits on `phase/d-gameplay`).** PR1 (`117323d`): `stat` model in api — `Stat` (base/min/max/`StatFormat` INTEGER/DECIMAL/PERCENT, clamp), `StatOperation`, `StatModifier` (add/multiply factories, finite+non-blank validation), `StatRegistry` (owner-scoped over `ContentRegistry<Stat>`); `content.spec.StatSpec` + `deserialize`/`toStat` = config type `stats`. PR2 (`7c0ca78`): `StatSource`, `StatSnapshot` (base → Σ ADD → ×(1+Σ MULTIPLY) → clamp; unknown stat ids ignored), `StatService` (`KEY` + `install` following `PlayerDataService`; per-player cache; `StatInvalidationListener` on held/armor/quit), built-in sources `ItemStatSource`/`BuffStatSource`/`PartyStatSource`/`RegionStatSource`, `ItemStats` (`ramcore:stats` `MapDataType`) + `ItemStackBuilder.stat(id, amount)`. PR3 (`42c7c1b`): `DamageCalculator` (crit → resistance clamped 0.95 → flat defense floored → lifesteal) → `DamageBreakdown`, `CombatStats` ids, `DamageProfile.Builder.withStats(..)`/`.calculator(..)`/`breakdown()`, `CombatControls.damage` untouched for non-opt-in. Deviations: no `RamCore`/`RamCoreConfig` wiring — `stat` is a consumer-installed service like `PlayerDataService` (config type = the `SpecLoader` deserializer), not a core-plugin feature; `RuleRegion` has no metadata map, so `RegionStatSource` keys modifiers by region id itself over `RegionTracker` membership; snapshots computed on the requesting thread (documented) rather than always hopped to the player scheduler. Tests: 27 across `StatTest`/`StatRegistryTest`/`StatSnapshotTest`/`DamageCalculatorTest` (api) and `StatSpecTest`/`StatServiceTest` (paper); full build green. Docs: API.md Stats section + combat example, MODULE_BOUNDARIES row, Package Map entry.
...
**Outcome (done 2026-09-11, four commits on `phase/d-gameplay`).** PR1 (`68a5261`): `ability` model in api — `AbilityTrigger`, `StatCost` (read-only stat gate; `affordable(StatSnapshot)`), `AbilityTargeting`+`AbilityTargets` (self/none/radius/nearest via `selector`, lookingAt via Paper ray-trace), `AbilityContext`, `AbilityAction` (functional run + default validate, mirrors `RewardAction`), `Ability` (immutable builder), `AbilityRegistry` (over `ContentRegistry<Ability>`). Effects reuse `presentation.PresentationEffect`. PR2 (`d2b0e72`): `AbilityCaster` state machine — gates cooldown→cost→validate; instant inline, channelled via `Schedulers.runLater`; `interrupt()`; `CastResult`/`AbilityCastStatus`. PR3 (`4aef934`): `AbilityService` (casters + item/hotbar/swap bindings + `install`), `AbilityTriggerModule` (Events API: interact/held/swap/quit), `AbilityCommandModule`, `AbilitySpec` (config type `abilities`; cooldown via `DurationParser`, castTicks, cost, targeting → `toAbility(id, action, effects)`). PR4 (`34f14fe`): `AbilityChannel`+`ChannelTick` (repeating channel via `runTimerTask`), `AbilityInterruptModule` (opt-in damage/move cancel), `AbilityCombo`+`ComboTracker` (window state machine → finisher), wired into `AbilityService`. Deviations: cooldowns tracked per ability via an injectable `Clock` rather than `CooldownTracker` (whose single base timeout + system clock fit neither per-ability durations nor deterministic tests); `AbilityAction`/effects are code-supplied not data (`AbilitySpec.toAbility` takes them) since behaviour is not expressible in config; interrupt module is opt-in (not auto-bound). All ability tests live in `ramcore-paper` because the model references Bukkit types (no paper-api on the api test classpath). Tests: 6 files (model, targeting, caster, service, spec, extensions); full build green. Docs: API.md Abilities section, MODULE_BOUNDARIES row, Package Map entry.
...
**Outcome (done 2026-09-11, three commits on `phase/d-gameplay`).** PR1 (`9c5f609`): primitives in `resourcepack` — `AssetSource` (lazy bytes from bytes/string/file/classpath), `PackFormats` (version→pack_format table + latest fallback), `Sha1` (digest+hex), `PackWriters` (pack.mcmeta / item-model definition / basic model). PR2 (`8b7672f`): `ResourcePackBuilder` (`item()`/`texture()`/`file()`, deterministic zip = sorted entries + fixed timestamps → stable SHA-1, `buildTo` → `PackBuildReport(added/changed/removed/unchanged/sha1Hex)` diffed against a per-file `PackManifest`, `metadata(uri, report)`), path-traversal guarded. PR3 (`728ee7e`): `ResourcePackHost` (JDK `HttpServer`, swappable pack) and `ResourcePackPromptSweeper` (scheduled `sweepTimeouts`, injectable clock). Deviations: JSON is hand-written and the manifest is a line-based `<sha1>\t<path>` file (not `pack-manifest.json`) because Gson is server-provided (`compileOnly`) and unusable off-server — this keeps the whole builder dependency-free and unit-testable in `ramcore-api` with `@TempDir`; no `ContentRegistrar`/`ItemSpec` change since the item side already applies item models via `ResourcePackItems.itemModel`; hosting/builder are consumer-driven (no `RamCore` wiring). Tests: primitives + builder + host in `ramcore-api` (pure, incl. real-HTTP GET), sweeper in `ramcore-paper` (FakeScheduler); full build green. Docs: API.md Resource Packs section extended + example, MODULE_BOUNDARIES note.
...
**Outcome (done 2026-09-11, three commits on `phase/d-gameplay`).** PR1 (`60d70a2`): model in api — `DialogueContext`, `DialogueAction`+`DialogueActions` (message/playerCommand/reward/openMenu/custom), `DialogueConditions` (`Predicate<DialogueContext>`: always/permission/metadataEquals), `DialogueChoice`, `DialogueNode` (builder, `availableChoices`), `Dialogue` (builder, validates start + choice targets), `DialogueRegistry`. PR2 (`b705ed9`): `DialogueSession` (start/choose/advance/end; enter runs node actions) with an injectable `DialoguePresenter` — `ChatDialoguePresenter` (Paper `ClickEvent.callback`) and `MenuDialoguePresenter` (`MenuView`). PR3 (`9e11292`): `DialogueSpec` (config type `dialogues`) + `Dialogues.onClick`/`onClickMenu` (`NpcClickHandler`) + docs. Deviations: conditions are `Predicate<DialogueContext>` directly (no dedicated interface); config `dialogues` covers the data-expressible subset (text/choices/permission/messages/commands) with reward/menu/custom actions code-supplied via `toDialogue`; objective `RUN_ACTION` linkage is a documented `custom` action (no dedicated executor — `ObjectiveTask.RUN_ACTION`/`ObjectiveTracker.apply` already exist); all tests in `ramcore-paper` (model references Bukkit `Player`). Tests: model, session traversal (recording presenter), spec round-trip; full build green. Docs: API.md Dialogue section + example, MODULE_BOUNDARIES row, Package Map entry.
...
**Outcome (done 2026-09-11, four commits on `phase/d-gameplay`).** PR1 (`b3038e2`): `reload` package in api — `ContentHashing` (stable SHA-1 of a node's merged value, sorted keys), `ContentSnapshot` (id→hash from a `ContentLoadResult`), `ContentDiff` (added/removed/changed/brokenReferences/rebuilt/failed/errors; `data(...)` + `withRebuild`). PR2 (`eda0e25`): `TemplateBound` (templateId/rebind/owner), `LiveObjectRegistry`, `ContentResolver`, `ContentPack`, `ContentReloadService` (`reload → Promise<ContentDiff>` via `Promise.supplyingAsync`; resolves added/changed, dispatches each rebind to its owner scheduler; failures collected). PR3 (`bfdf106`): `/ramcore diagnostics reload <pack>` subcommand (optional service dep, non-blocking `TaskContext.async` continuation), `RamCore.reloadService()` shared instance wired into the diagnostics module + docs. Deviations: `diff.failed` = resolution failures (synchronous, in-diff); rebind-thread exceptions are caught + logged (not in the returned diff, since they may complete after it); node hash uses `node.raw()` canonicalised (no Configurate render helper exists). All reload tests: snapshot/diff pure in `ramcore-api`, service in `ramcore-paper` (FakeScheduler — `runAll()` before `join()` to avoid a single-thread deadlock); full build green. Docs: API.md Hot-Reloadable Content section, MODULE_BOUNDARIES row, Package Map entry.
...
**Outcome (done 2026-09-11, two commits on `phase/d-gameplay`).** PR1 (`2688dc5`): `schedule` package in api — `CronExpression` (in-house 5-field parser: `*`/values/ranges/lists/steps, dow 0-6 with 0/7=Sunday, dom-OR-dow rule, `next` search) and `Schedule` (`nextAfter(Instant, ZoneId)` + `cron`/`every`/`at`). PR2 (`4ef7c93`): `Job`, `JobState` (lastRun/nextRun epoch millis), `MissedRunPolicy` (skip/catchUp(max)), `RealTimeScheduler` (once-per-second async ticker → due jobs run on their `TaskContext`; `register(job, previousState)` applies the missed-run policy; guarded execution) + docs. Deviation: `JobState` is held in memory with an injectable `Clock`; `Store<String, JobState>` persistence is seeded via `register(job, state)` rather than owned by the scheduler (keeps the runner off the async store path and unit-testable). Tests: cron/schedule pure in `ramcore-api`; runner in `ramcore-paper` (FakeClock + FakeScheduler). Full build green. Docs: API.md Real-Time And Cron Scheduling section, MODULE_BOUNDARIES row, Package Map entry.
...
**Outcome (done 2026-09-11, three commits on `phase/d-gameplay`).** PR1 (`e4fd791`): `worldinstance` file primitives in api — `InstanceMarker` (`ramcore-instance.json` write/read/detect, hand JSON) and `WorldInstances` (`copyTemplate` skipping uid.dat/session.lock, `sweep`, `deleteRecursively`). PR2 (`9984b0f`): `WorldBackend` seam (`supportsInstances`/`worldContainer`/`load`/`unload`/`evacuate`), `WorldInstanceOptions`, `WorldInstance` (Terminable: `closeAsync` evacuate→unload→delete), `WorldInstanceService` (`create` copies+marks off-thread then loads via backend, gated on `supportsInstances`; `sweepStartup`). PR3 (`85db52c`): `PaperWorldBackend` (ramcore-paper: `Bukkit.createWorld`/`unloadWorld` on global, `teleportAsync` evacuation, Folia-false gate) + docs. Deviations: the capability gate is `WorldBackend.supportsInstances()` rather than an added `NmsCapability.WORLD_INSTANCES` enum value (avoids perturbing the capability self-test matrix); `RuleRegion` boundary + `PartyGroup`/`EncounterInstance` binding are left to the consumer (`WorldInstance` is a `Terminable` to bind) rather than built in. Tests: file ops + service via an in-memory fake backend (`ramcore-api`/`ramcore-paper`, FakeScheduler; `runAll()` before `join()`). Paper backend is server-only (manual smoke test). Full build green. Docs: API.md Instanced Worlds section, MODULE_BOUNDARIES row, Package Map entry.
