...
- `AbilityTargeting` + `AbilityTargets` — `self`, `none`, `radius`, `nearest` (via `selector`), `lookingAt` (strict Paper ray-trace), plus the forgiving-aim strategies (3.9): `rayTrace(range, raySize)` (fat-ray single target), `cone(range, angleDeg)` / `nearestInCone` (half-angle cone about the look axis), `beam(range, width)` (line AoE around the look ray), and `groundRadius(range, radius)` (aimed block → AoE around that point). Cone/beam geometry is the pure, unit-tested `AbilityGeometry`; entity queries stay in `AbilityTargets`.
...
- Targeting feedback (3.9, experimental): `ability.telegraph.Telegraph` (`Terminable show(Player)`) with `Telegraphs` built-ins — particle shapes `ring`/`cone`/`line`/`blockMarker` (redraw on the caster's or the marker location's scheduler, skip a frame for an invalid caster), `glow(targeting)` (server-side `setGlowing` lock-on, restored on close), and `all(...)`. `Ability.builder.telegraph(Telegraph)` shows the preview while a cast/channel runs and clears it on completion or interrupt. `AbilityAiming.begin(player, targeting, telegraph[, AimOptions])` enters interactive aim: the preview and glow follow the look each refresh; a click/swap-hands confirms and resolves targets into the returned `Aiming.result()` (`Promise<List<LivingEntity>>`); moving, quitting, or the timeout cancels it (promise completes exceptionally). All rendering runs on the player/entity/region scheduler (Folia-safe); everything here is server-side (no client mod, no HUD crosshair).
