...
- [x] Add resource pack helper utilities.
...
- [x] Add an instanced loot toolkit.
...
- [x] Define an NMS access strategy before adding more internals.
...
- [x] Build a mob AI facade on top of Paper Goals, with NMS fallbacks for missing behavior.
...
- [x] Explore NMS Brain, Memory, Sensor, and Activity adapters for modern mobs.
...
- [x] Improve pathfinding utilities beyond Paper's basic Pathfinder API.
...
- [x] Add advanced entity control utilities.
...
- [x] Add attribute and combat behavior helpers.
...
- [x] Build item data component utilities around Paper's experimental Data Components API.
...
- [x] Expand item NBT and serialization support.
...
- [x] Add world and block internals where Paper API is thin.
...
- [x] Add loot table and trade manipulation utilities.
...
- [x] Add packet-level utilities only where ProtocolLib or Paper events are insufficient.
...
- [x] Add NMS test and compatibility infrastructure.
...
- [x] Improve the data repository layer.
  - Add simple file-backed repositories first.
  - Consider async save queues, dirty tracking, and migration hooks.

- [x] Improve event subscription utilities.
  - Support automatic lifecycle cleanup, filtered listeners, one-shot listeners, priority helpers, and terminable binding.

## Release Readiness

- [x] Confirm Java 25 is acceptable for downstream consumers.
- [x] Test the plugin manually on both Folia and plain Paper.
- [x] Review shade warnings and optionally exclude duplicate dependency metadata.
- [x] Decide whether diagnostics should remain enabled by default for public releases.
- [x] Define the RamCore 2.0 module boundaries before implementing platform features.
- [x] Decide which features belong in core, optional modules, integrations, or examples.
- [x] Define API stability levels for stable, experimental, Paper-experimental, and NMS-backed APIs.
- [x] Document which 2.0 APIs are Folia-safe by design and which require explicit scheduler context.
